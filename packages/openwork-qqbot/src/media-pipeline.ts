...
export interface DefaultQQMediaPipelineOptions {
  transcribeVoice?: (input: { attachment: QQInboundAttachment; localPath?: string }) => Promise<string | undefined>
}
...
export class DefaultQQMediaPipeline implements QQMediaPipeline {
  ...
  private async materializeAttachment(
    ...
    const localPath = attachment.localPath || (await this.downloadAttachment(message.messageId, sourceUrl, fileName))
  ...
  private async downloadAttachment(messageId: string, sourceUrl: string | undefined, fileName: string): Promise<string | undefined> {
    if (!sourceUrl) {
      return undefined
    }
  ...
  private async tryTranscribe(
    ...
    try {
      ...
      form.append("file", new Blob([fileBuffer], { type: attachment.contentType || "audio/wav" }), path.basename(localPath))
